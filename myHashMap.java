...
class myHashMap<K,V> {
    ...
    public V remove(K key) {int bucketIndex = getBucketIndex(key);
        HashNode<K, V> head = bucket.get(bucketIndex);
        HashNode<K, V> prev = null; // last node
    ...
    public boolean replace(K key, V oldVal, V newVal) {
        ...
        if ((currentVal == null && oldVal == null) || (currentVal != null && currentVal.equals(oldVal))) {
            // Use replace(K key, V newVal) for code reuse
            replace(key, newVal);
            return true;
        }
